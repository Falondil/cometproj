...
for j in range(number_of_loops): # Divide this into Scheme numbering
    ...
    # new_phi = new_phi - new_phi[-1] # Shifting all phi so phi[-1] = 0
    ...
    new_deps = depscalc(new_eps)
    new_F = new_F[sortind] # sort new F via same sorting
    
    # now resample F
    # depstoobig = (new_deps>2).nonzero()[0] # larger than 2 neutral energies
    # new_eps = np.insert(new_eps, depstoobig, new_eps[depstoobig]-np.sign(new_eps[depstoobig])*new_deps[depstoobig]/2)
    ...
    # 7. Overwrite all old values
    old_density = new_density
    old_phi = new_phi
    old_F = new_F
    eps = new_eps # This is wrong when only resampling.
    deps = new_deps # This is wrong when only resampling
