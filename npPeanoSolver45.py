...
excess = 7*beta  # how many electrontemperatures we consider before truncation
eps, deps = np.linspace(-excess, excess, int(10*excess), retstep=True) # centered on 0.
...
def delphiG0(eps, G0, V0, density): # calculates the change in potential using G0 as an interpolated function
    def subfunc(delp):
        ...
        return V@G0(eps+np.divide(np.sum(np.matrix.transpose(delp*Vtrans*x_k[1:-1]**2), axis=0), np.sum(np.matrix.transpose(Vtrans*x_k[1:-1]**2), axis=0), out=np.zeros_like(eps), where=np.sum(V, axis=0)!=0))-density
    sol = optimize.fsolve(subfunc, np.zeros_like(density)) # find which change in potential delp solves the subfunction
    return sol
...
for j in range(number_of_loops):
    ...
    ax4.semilogy(x_k[1:-1], ratiovalue, '.', color='k')
