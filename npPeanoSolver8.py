...
def dUphasedeps(Vmat):
    L = np.sum(LI(Vmat), axis=0) # compute lower integral, x axis = 0
    return 16*pi**2*2**(1/2)*L

# def F1(V1, wide_density): # calculates the unitless distribution function by solving for V matrix
#     new_F, res, rank, sing = np.linalg.lstsq(4*pi*2**(1/2)*V1*deps, wide_density, rcond=None) # V1 is square matrix, wide_density has a value for all x_k (including inner and outer edge points)
#     return new_F

# Non-negative solutions for F are selected
def F1(V1, wide_density): # calculates the unitless distribution function by solving for V matrix
    new_F, res = nnls(4*pi*2**(1/2)*V1*deps, wide_density) # V1 is square matrix, wide_density has a value for all x_k (including inner and outer edge points)
    return new_F
...
def arraytimeevaluator(v0, a, s): # Calculates the crossing times for an array of initial velocities, accelerations and the signed shell boundary distance 
    ...
    taylor_bool = abs(2*a*s)<=abs(0.01*v0**2)
    ...
    t[complex_ind] = v0[complex_ind]/a[complex_ind]*(-1+(1+2*a[complex_ind]*s_used[complex_ind]/v0[complex_ind]**2)**(1/2))
...
for j in range(number_of_loops): # Divide this into Scheme numbering
    ...
    del_F = delF(new_Vmat)
    new_F = old_F + del_F
    # new_F = F1(new_Vmat, new_density) # uses nnls
    ...
    # plt.figure()
    # plt.title('Validity of nnls method. Iteration: '+str(counter))
    # plt.ylabel('Electron density/Ion density')
    # plt.xlabel('Distance from comet center [R'+'$_{C}$]')
    # plt.semilogy(x_k, electron_density/new_density, '.', color='k')
