...
eps, deps = np.linspace(-excess, excess, int(9*excess), retstep=True) # centered on 0.
...
def doublezerodist(energy, fi0): # computes a ...
    return -energy*abs((fi0+energy))**2*np.exp(-energy/fi0)
...
def delphiG0(eps, G0, V0, density, guess=np.zeros_like(x_k[1:-1])): # calculates the change in potential using G0 as an interpolated function
    def subfunc(delp):
        ...
        return x_k[1:-1]*((V@G0(eps+np.divide(np.sum(np.matrix.transpose(delp*Vtrans*x_k[1:-1]**2), axis=0), np.sum(np.matrix.transpose(Vtrans*x_k[1:-1]**2), axis=0), out=np.zeros_like(eps), where=np.sum(V0, axis=0)!=0))))-x_k[1:-1]*(density)
...
for j in range(number_of_loops):
    ...
    # new_F = np.divide(np.interp(eps, new_eps, old_F*np.sum(LI(new_Vmat), axis=0)*new_deps), deps*Vx2int, out=np.zeros_like(old_F), where=Vx2int!=0)     
    ...
    fig.suptitle('Timestep: '+str(counter)+', Electrons: '+str(electroncounter(new_F, halfnew_Vmat))+', Ions: '+str(len(ionmatrix)))
    ...
    ax4.set_title('Electron vs. ion density')
    ax4.semilogy(x_k[1:-1], densityfraction, '.', color='k')
    ax4.set(xlabel='Distance from comet center [R'+'$_{C}$]', ylabel='Density ratio')
    fig.tight_layout()
