...
def delphiG0(eps, G0, V0, density, guess=np.zeros_like(x_k[1:-1])): # calculates the change in potential using G0 as an interpolated function
    def subfunc(delp):
        ...
        # return np.log((V@G0(eps+np.divide(np.sum(np.matrix.transpose(delp*Vtrans*x_k[1:-1]**2), axis=0), np.sum(np.matrix.transpose(Vtrans*x_k[1:-1]**2), axis=0), out=np.zeros_like(eps), where=np.sum(V0, axis=0)!=0)))/(1+x_k[1:-1]))-np.log((density)/(1+x_k[1:-1]))
...
for j in range(number_of_loops):
    ...
    ax1.axhline(electrontemperature/10, color='b')
