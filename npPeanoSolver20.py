...
for j in range(number_of_loops):
    ...
    xbprev = x_e[-1] # remember previous barrier limiter
    x_e = np.arange(x_comet, furthestion, x_thickness) # calculate new spatial range for electrons
    xb = x_e[-1]
    delxb = x_e[-1]-xbprev # calculate how far the barrier has moved
    ...
    del_density = (new_density - old_density)[:int(xb)] # makes the change in density take the shape x_e. Neglect last element since that is where the barrier is placed
