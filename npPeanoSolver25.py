...
# 1.5 Electrons
excess = 5*beta # how many electrontemperatures we consider before truncation
eps, deps = np.linspace(-excess, excess, int(2*excess), retstep = True) # centered on 0.

def loginitialrho(eps, Vmat):
    inv2phi = 1/(2*phi_at_comet)
    xterm = x_k**2*np.matrix.transpose(Vmat)*(erf(np.sqrt(inv2phi+np.log(x_k)))-erf(np.sqrt(inv2phi)))
    xterm = np.matrix.transpose(xterm)
    cterm = 2**(1/2)/(phi_at_comet**2)*n_per_shell/Del_t*np.exp(inv2phi)*np.exp(-eps/phi_at_comet)*1/x_k[-1]
    F = np.sum(xterm*cterm, axis=0) # compute integral, x axis = 0
    return F

# def loginitialrho(eps):
#     xlin, dx = np.linspace(1, x_k[-1], 10000, retstep=True) # creates a linearly spaced np array of x values
#     philin = phi_at_comet*np.log(x_k[-1]/xlin) # creates the potentials corresponding to each comet distance described by xlin
#     Vmat = Vmatrix(eps, philin) # creates the speed V at each x
    
#     inv2phi = 1/(2*phi_at_comet)
#     cterm = 2**(1/2)/(phi_at_comet**2)*n_per_shell/Del_t*np.exp(inv2phi)*np.exp(-eps/phi_at_comet)*1/x_k[-1]
#     xterm = xlin**2*np.matrix.transpose(Vmat)*(erf(np.sqrt(inv2phi-np.log(xlin)))-erf(np.sqrt(inv2phi)))
#     xterm = np.matrix.transpose(xterm)
    
#     F = np.sum(cterm*xterm*dx, axis=0) # compute integral, x axis = 0
#     return F
...
nionstart = int(initialionnumber(number_of_shells*10000+1)) # 49012
ionmatrix=initialcreation(nionstart)
old_phi = phi_anders_log
