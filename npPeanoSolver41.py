...
# rectangular integration of logiondensity between x = [1, xmax]
def initialionnumber(number_of_points):
...
def initialcreation(number_of_ions):
...
def CDF(func, arglist):  # calculates CDF of a function at linearly spaced points (arglist)
    # numpy array of function evaluated at each arg in arglist
    funclist = func(arglist)
    # calculates an array consisting of the cumulative sum up to and including arg
    csum = np.cumsum(funclist)
    return csum/csum[-1]
...
def maxwell(Vmat): # computes the maxwell boltzman distribution
    cterm = 2/(np.sqrt(pi)*beta**(3/2))
    xterm = Vmat*np.exp(-Vmat**2/beta)
    return cterm*xterm

def sigmoidandersrho(xlist):
    phi = phi_anders_sigmoid(xlist)
    u1 = np.sqrt(1+2*phi0-2*phi) # maximal ion speed
    c = 1+np.exp((1-L)/b)
    coef = 2**(3/2)*b
    rt1 = np.sqrt(abs(2-4*phi)) # root term 1
    rt2 = np.sqrt(abs(4*c*phi0-4*phi+2)) # root term 2

    # xterm = 1/xlist**2*np.arctanh(u1*2**(1/2)/)
    quit

# def sigmoidinitialrho(eps, Vmat):
...
def densitydifferencesingle(del_phi, V0, F, iondensity): # calculate the difference in electron and ion density at a given position
    return etaesingle(F, V0, del_phi) - iondensity
...
def delphisingle(V0, F, iondensity):
    sol = optimize.newton(densitydifferencesingle, 0, args=(V0, F, iondensity), maxiter=500)
    return sol

def delphisingle2(V0, F, iondensity):
    def densitydifferencesingle2(delp):
        return etaesingle(F, V0, delp) - iondensity
    sol = optimize.newton_krylov(densitydifferencesingle2, 0)
    return sol
...
old_density = np.divide(logiondensity(x_k), 4*pi*x_k**2)
old_gooddensity = np.divide(logiondensity(xenergy), 4*pi*xenergy**2) # calculate the density in as many points as the discretization in energy
...
energyphi = phi_anders_log(xenergy) # potential at the points xenergy
...
for j in range(number_of_loops):
    ...
    new_F = electrondeleter(old_F, Vmat, ionsvanished)
    new_F[int(len(new_F)/2):] = 0
    
    # find highest energy of the remaining electrons
    highestenergy = eps[new_F.nonzero()[0][-1]]
    ...
    for i in range(len(idensity)):
        del_phi[i] = delphisingle2(ionVmat[i+1,:], new_F, idensity[i])
    ...
    # calculate new potential at points xe
    new_electronphi = np.interp(xe, x_k, new_ionphi)
    # calculate change in potential at points xe
    del_electronphi = new_electronphi-electronphi
