...
def Fper2V(F0, Vmat): # Calculates integral corresponding to change in unitless potential. (Denominator of RHS in expression for del_phi)
    ...
    ret = np.sum(2*pi*2**(1/2)*mat, axis=1) # approximate integral via summation over eps axis = 1 # Gör så att den summeras bara över positiva eps
    # mat[:,:int(len(eps)/2)]
    return ret
...
number_of_loops = 50# neutral_time
