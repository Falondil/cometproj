...
def G0func(eps, F, depsilon=deps): # creates the function G(eps)
    return interp1d(eps, 4*pi*2**(1/2)*F*depsilon, bounds_error=False, fill_value=0) # piecewise linear
    # return PchipInterpolator(eps, 4*pi*2**(1/2)*F*depsilon, extrapolate=True) # piecewise cubic hermite interpolating polynomial
...
def delphimulti(eps, F0, V0, density): # caclculates the change in potential using F(eps0) = F0(eps0) approximation
    def Z(delp): # create subfunction for the function that we want root solution to
        ...
        Vsquared = (Vsquared + abs(Vsquared))/2 # all negative values become 0
        V = Vsquared**(1/2)
        return 4*pi*2**(1/2)*np.sum(F0*V, axis=1)-density
...
for j in range(number_of_loops):
    ...
    new_F = old_F
    # new_F = electrondeleter(new_F, Vmat, ionsvanished)
    ...
    # Gf = G0func(eps, new_F)
    # del_phi = delphiG0(eps, Gf, ionVmat[1:-1,:], idensity)
