...
phi_anders = phi_at_comet*(x_k[-1]-x_k)/x_k[-1]
...
# centered on 0.
eps, deps = np.linspace(-excess, excess, int(2*excess), retstep=True)
...
def loginitialrho(eps, Vmat):
    ...
    xterm[0] /= 2
    xterm[-1] /= 2
    cterm = 2**(1/2)/(phi_at_comet**2)*n_per_shell/Del_t * \
        np.exp(inv2phi)*np.exp(-eps/phi_at_comet)*1/xe[-1]
...
def delphisingle(V0, F, iondensity):
    sol = optimize.newton(densitydifferencesingle, 0, args=(V0, F, iondensity), maxiter=200, tol=1e-3)
    return sol
...
ionphi = phi_anders_log
# potential linearly interpolated to electron space discretization xe
electronphi = np.interp(xe, x_k, ionphi)
