...
def arraytimeevaluator(v0, a, s, s_alt): # Calculates the crossing times for an array of initial velocities, accelerations and the two nearest shell boundary distances 
    ...
    t[complex_ind] = v0[complex_ind]/a[complex_ind]*(-1+(1+2*a[complex_ind]*s_alt[complex_ind]/v0[complex_ind]**2)**(1/2))
    s_used[complex_ind] = s_alt[complex_ind] # alternate boundary is reached instead
...
# old_phi = np.zeros_like(x_k) # alternative start where the first phi is set twice
...
old_density = 4*pi*2**(1/2)*Vmat@old_F*deps # Peano equation
