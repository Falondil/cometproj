...
eps, deps = np.linspace(-excess, excess, int(6*excess), retstep = True) # centered on 0.
...
def delW(Vmat, xb, delxb): # xb is the position of the barrier, xb-1 is the index such that x_k[xb-1] = xb. 
    Vxb = Vmat[xb-1, :] # calculate V at the barrier
    xtrunc = x_k[:xb] # calculate x range limited by the barrier
    Vtrunc = Vmat[:xb, :] # truncate Vmatrix at the barrier, Should Vxb be included or just excluded?
    Vtruncx2 = np.matrix.transpose(np.matrix.transpose(Vtrunc)*xtrunc**2*x_thickness)
    sumVtruncx2 = np.sum(Vtruncx2, axis=0) # compute integral over x
    rightint = np.divide(xtrunc**2, Vtrunc, out=np.zeros_like(xtrunc), where=Vtrunc!=0)
    numer = 2/3*delxb*xb**2*(3/2*Vxb*sumVtruncx2-Vxb**3*rightint)
    denom = sumVtruncx2**2
    return np.divide(numer, denom, out=np.zeros_like(numer), where=denom!=0)
...
number_of_loops = 30# neutral_time
...
for j in range(number_of_loops):
    ...
    x_e = np.arange(x_comet, furthestion, x_thickness)
    ...
    for index in range(10):
        ...
        del_phi = delphi(Vmat, new_F, del_density)
        ...
        # Vmat = new_Vmat # unsure. This probably shouldn't be here
    ...
    avg_e_energy = averageelectronenergy(new_F, new_Vmat)
